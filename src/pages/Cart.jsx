...
function Cart() {
  ...
  const onClearCart = () => {
    if (window.confirm("Ты уверен в этом?")) {
      dispatch(clearCart())
    }
  }
          ...
          <div className="cart">
            ...
            <div className="content__items">
              {addedPizzas.map((obj, index) =>
                <CartItem key={`${obj.name}_${index}`}  {...obj} onMinus={onMinusItem} onPlus={onPlusItem} onRemoveItem={onRemoveItem} totalCount={items[obj.id].items.length} totalPrice={items[obj.id].totalPrice} />
              )}
            </div>
